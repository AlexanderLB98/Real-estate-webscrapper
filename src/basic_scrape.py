...
def basic_scrape():
    ...
    ids = []
    df = pd.DataFrame(
        columns=["ID", "Título", "Enlace", "Precio", "Detalles", "Descripción"]
    )
    ...
    while True:
        ...
        for article in articles:
            ...
            full_link = (
                f"https://www.idealista.com{href}" if href != "No disponible" else href
            )
            ...
            details_text = (
                details.get_text(" | ", strip=True) if details else "No disponible"
            )

            # Descripción
            description = article.find("p", {"class": "ellipsis"})
            description_text = (
                description.get_text(strip=True) if description else "No disponible"
            )
